%% uctest.tex 11/3/94
%% Copyright (C) 1988-2004 Daniel Gildea, BBF, Ethan Munson.
%
% This work may be distributed and/or modified under the
% conditions of the LaTeX Project Public License, either version 1.3
% of this license or (at your option) any later version.
% The latest version of this license is in
%   http://www.latex-project.org/lppl.txt
% and version 1.3 or later is part of all distributions of LaTeX
% version 2003/12/01 or later.
%
% This work has the LPPL maintenance status "maintained".
%
% The Current Maintainer of this work is Daniel Gildea.
%
% 2007/08/01
% LaTeX Package "ucr" is modified from LaTeX package "ucthesis."
% This modification is therefore under to the conditions of
% the LaTeX Project Public License.
% Its formality is suitable for the thesis of Universty of
% California, Riverside.
% This test document is for the convenience of all students of
% Universty of California, Riverside.
% Contact Charles Yang at chcyang@yahoo.com if you like.
% Charles Yang has nothing to do with the original author's sarcasm.
%
% \documentclass[11pt]{ucthesis}
% \documentclass[11pt]{ucr}
\documentclass[oneside,final, letterpaper]{ucr}
\usepackage[backend=biber,sortcites=true]{biblatex}
\addbibresource{bibfile.bib}
\usepackage[hidelinks]{hyperref}
\newcommand{\sect}[1]{\chapter{#1}}
\newcommand{\subsect}[1]{\section{#1}}
\newcommand{\subsubsect}[1]{\subsection{#1}}
\newcommand{\subsubsubsect}[1]{\subsubsection{#1}}
\newcommand{\subsubsubsubsect}[1]{\paragraph{#1}}
\input{packages}
\begin{document}

% Declarations for Front Matter

\title{\input{title}}
\author{Clarity Laska Shimoniak}
\degreemonth{June}
\degreeyear{2025}
\degree{Master of Science}
\chair{Dr. Philip Brisk}
\othermembers{Dr. Daniel Wong\\
Dr. Ahmed Eldawy}
\numberofmembers{3}
\field{Computer Engineering}
\campus{Riverside}

\maketitle
\copyrightpage{}
\approvalpage{}

\degreesemester{Fall}

\begin{frontmatter}

\begin{acknowledgements}
\input{acknowledgments}
\end{acknowledgements}

\begin{dedication}
\null\vfil
{\large
\begin{center}
\input{dedication}
\end{center}}
\vfil\null
\end{dedication}

\input{abstract}

\tableofcontents
\listoffigures
\listoftables
\end{frontmatter}

% \part{First Part}


\input{content}



% \nocite{*}
{\ssp%
\setlength\bibitemsep{0.5\baselineskip}%
\printbibliography%
}

\input{appendix}


\end{document}
